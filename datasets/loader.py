...
_allowed_label_types = (
    ...
    # coarse labels for CIFAR100 dataset.
    "coarse",

    # fine labels for CIFAR100 dataset.
    "fine",
)
...
class CIFARException(Exception):
    pass


class CIFAR(object):
    def __init__(self, path='.', datatype="cifar10", label_type="coarse", grayscale=False, mode='vanilla'
                 , return_type='lists'):
        self.path = os.path.join(path, datatype)
        self.datatype = datatype
        self.grayscale = grayscale

        assert label_type in _allowed_label_types, \
            "selected label type '{}' not in {}".format(
                label_type,
                _allowed_label_types
            )
        ...
        if datatype == "cifar10":
            self.test_fname_list = ["test_batch"]
            self.train_fname_list = ["data_batch_1", "data_batch_2", "data_batch_3", "data_batch_4", "data_batch_5"]
        elif datatype == "cifar100":
            self.test_fname_list = ["test"]
            self.train_fname_list = ["train"]
    ...
    def load_training(self):
        ...
        ims = []
        labels = []
        for file in self.train_fname_list:
            batch_dict = self.cifar_loader(os.path.join(self.path, file))

            ims.append(batch_dict[b"data"])
            if self.datatype == "cifar10":
                labels.append(batch_dict[b"labels"])
            elif self.label_type == "coarse":
                labels.append(batch_dict[b"coarse_labels"])
            elif self.label_type == "fine":
                labels.append(batch_dict[b"fine_labels"])

        ims = _np.concatenate(ims, axis=0)
        labels = _np.concatenate(labels, axis=0)

        if self.grayscale:
            ims = self.convert_to_grayscale(ims)
    ...
    def load_testing(self):
        ...
        ims = []
        labels = []
        for file in self.test_fname_list:
            batch_dict = self.cifar_loader(os.path.join(self.path, file))
    ...
    def convert_to_grayscale(self, images):
        return 0.299 * images[:, :1024] + 0.587 * images[:, 1024:-1024] + 0.114 * images[:, -1024:]
    ...
    def process_images(self, images):
        if self.return_type is 'lists':
            ...
            raise CIFARException("unknown return_type '{}'".format(self.return_type))
    ...
    def process_images_to_numpy(self, images):
        ...
        if self.mode == 'vanilla':
            ...
            raise CIFARException("unknown mode '{}'".format(self.mode))

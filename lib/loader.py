"""
Functions to load and preprocess MNIST datasets
Taken from:
https://gitlab.com/cwalder/linkgistic/-/blob/master/mnist/loader.py
"""
import gzip
import os
import random
...
class MNIST(object):
    def __init__(
        ...
        assert mode in _allowed_modes, "selected mode '{}' not in {}".format(
            mode, _allowed_modes
        )
        ...
        assert (
            return_type in _allowed_return_types
        ), "selected return_type '{}' not in {}".format(
            return_type, _allowed_return_types
        )
    ...
    def load(self, path_img, path_lbl, batch=None):
        if batch is not None:
            if type(batch) is not list or len(batch) is not 2:
                raise ValueError(
                    "batch should be a 1-D list" "(start_point, batch_size)"
                )
        ...
        for i in range(size):
            ...
            if self.emnistRotate:
                ...
                for r in range(rows):
                    subs.append(
                        x[(rows - r) * cols - cols : (rows - r) * cols]
                    )
